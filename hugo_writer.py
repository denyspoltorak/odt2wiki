...
class HugoMarkdownWriter(md_writer.MarkdownWriter):
            ...
            for i in toc.items:
                ...
                if i.level < max_level:
                    ...
                    if isinstance(picture, plugins.Wide):
                        ...
                        output.extend(self._add_picture(picture, self._make_image_presentation(picture)))
    ...
    def _make_metadata(self, creator):
        ...
        primary_image = self._customization.get_primary_image(creator)
        if(primary_image):
            assert not primary_image.endswith(".svg")
            output.append(f'primary_image = "{self._escape_link(primary_image)}"')
        # ToC
